...
cvg_double PID::getOutput() {
	...
	if (actuatorSaturation >= 0.0) {
		...
		if ((output < actuatorSaturation && output > -actuatorSaturation) ||
			(output >= actuatorSaturation && error < 0) ||
			(output <= -actuatorSaturation && error > 0)
			)
