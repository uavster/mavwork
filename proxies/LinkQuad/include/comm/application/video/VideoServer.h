...
//		StaticBuffer<_FrameT, 1> sendHeaderBuffer;
	...
	cvg_double outputScale;

	IplImage *imageBeforeScaling;
	IplImage *imageAfterScaling;
...
public:
	inline VideoServer() { outputFormat = JPEG; outputQuality = VIDEOSERVER_DEFAULT_QUALITY; serverEventHandler.setParent(this); imageBeforeScaling = imageAfterScaling = NULL; }
	virtual ~VideoServer() {}

	virtual void open(const IAddress &addr, cvg_int maxQueueLength = -1);
	virtual void close();

	void setOutputFormat(VideoFormat format, cvg_double quality = VIDEOSERVER_DEFAULT_QUALITY, cvg_double scale = 1.0);
	inline VideoFormat getOutputFormat() { return outputFormat; }
	void feedFrame(const _FrameT *header, VideoFormat inputFormat, const void *frameData, cvg_int frameDataLength, cvg_int width, cvg_int height, cvg_uint *actualSize, cvg_uint *actualWidth, cvg_uint *actualHeight);

	cvg_int getNumClients();
};
